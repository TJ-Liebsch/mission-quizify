...
if __name__ == "__main__":
    ...
    with screen.container():
        ...
        with st.form("Load Data to Chroma"):
            ...
            if submitted:
                ...
                # Forge a chroma collection
                document = chroma_creator.query_chroma_collection(topic_input) 
                
                # # Utilize the Chroma collection to extract relevant information
                # relevant_info = chroma_creator.extract_information(topic_input)

                # # Step 2: Use the input query to perform a semantic search against the Chroma collection
                # semantic_search_results = chroma_collection.semantic_search(user_query)

                # # Step 3: Retrieve the relevant information based on the semantic search results
                # relevant_info = []
                # for result in semantic_search_results:
                #     relevant_info.append(result.document_content)  # Assuming the relevant information is stored in the 'document_content' attribute
    ...
    if document:
        ...
        with st.container():
            ...
            # # Display the relevant information to the user
            # st.write("Relevant Information based on the Query:")
            # for info in relevant_info:
            #     st.write(info)
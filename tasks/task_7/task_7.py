...
class QuizGenerator:
    ...
    def init_llm(self):
        ...
        self.llm = VertexAI(
            ############# YOUR CODE HERE ############
            model_name = "gemini-pro", # name of the model
            temperature = 0.3, # Controls the randomness of the output (lower temp = less random)
    ...
    def generate_question_with_vectorstore(self):
        ...
        # This verifies the LLM and vectorstore are initalized and available
        if self.llm is None:
            # LLM is not initialized
            raise ValueError("LLM is not initialized. Please initialize it before generating quiz questions.")
        
        if self.vectorstore is None:
            # Vectorstore is not initialized
            raise ValueError("Vectorstore is not initialized. Please initialize it before generating quiz questions.")    
        ...
        # HINT: Use the vectorstore initialized on the class as the retriever
        ############# YOUR CODE HERE ############
